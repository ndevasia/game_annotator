...
function getParticipantsFromS3Data() {
  /**
   * Reads s3_data folder and returns usernames that start with 'participant_'
   */
  try {
    const s3DataPath = path.join(__dirname, 's3_data');
    if (!fs.existsSync(s3DataPath)) {
      console.warn('⚠️ s3_data folder not found');
      return [];
    }

    const entries = fs.readdirSync(s3DataPath, { withFileTypes: true });
    const participants = entries
      .filter(entry => entry.isDirectory() && entry.name.startsWith('participant_'))
      .map(entry => entry.name)
      .sort();

    console.log(`📋 Found ${participants.length} participants in s3_data:`, participants);
    return participants;
  } catch (err) {
    console.error('Error reading s3_data folder:', err);
    return [];
  }
}

function createAdminSelectWindow() {
  return new Promise((resolve) => {
    adminSelectWindow = new BrowserWindow({
      width: 550,
      height: 600,
      modal: true,
      show: false,
    ...
    adminSelectWindow.loadFile('admin-select.html');

    adminSelectWindow.once('ready-to-show', () => {
      adminSelectWindow.show();
    });

    ipcMain.once('admin-mode-selected', async (event, { mode, participant }) => {
      adminMode = mode === 'admin';
      adminParticipant = participant;
      console.log(`👤 Admin mode: ${adminMode}, Participant: ${adminParticipant || 'none'}`);
      adminSelectWindow.close();
      resolve();
    });

    adminSelectWindow.on('closed', () => {
      adminSelectWindow = null;
      // If window closed without selection, default to normal mode
      if (!adminMode && !adminParticipant) {
        adminMode = false;
...
ipcMain.handle('get-admin-participants', () => {
  return getParticipantsFromS3Data();
});

function createUsernamePrompt() {
  return new Promise(async (resolve) => {
    // If in admin mode, use the selected participant as username
    if (adminMode && adminParticipant) {
      console.log(`👤 Setting username to admin participant: ${adminParticipant}`);
      sessionMetadata.setUsername(adminParticipant);
      return resolve();
    }
